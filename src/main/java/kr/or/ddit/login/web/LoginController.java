...
public class LoginController {
	...
	// ranger()가 먼저 호출되고, Model 객체에 rangers라는 이름의 속성이 들어간다.
	// 이후에 json()이라는 메서드가 호출된다.
	// json() 입장에서 Model 객체에 rangers라는 속성이 존재하게 된다.
	@RequestMapping("/json")
	public String json() {
		return "jsonView"; // <bean id="jsonView" class="MappingJackson2JsonView"/>

		// 현재 ViewResolver를 2개 등록해 놓았다.
		// 1순위는 BeanNameViewResolver -> ViewName에 해당하는 bean이 있는지를 찾는다.
		// 만약 해당하는 bean(view)이 있으면 해당 view 결과를 생성한다.
		// 2순위는 InternalResourceViewResolver
		// prefix, suffix 설정에 따라서 /WEB-INF/views/jsonView.jsp로 forwarding
		// internalResourceViewResolver는 view이름에 해당하는
		// 자원이 존재하는지 유무를 체크하지 않고 무조건 forwarding한다.
		// -> viewResolver의 우선순위를 가장 후순위로 미뤄야 한다 !
	}
